...
def run_coverage(
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # Accurate footprint-fit branch: rotate the actual robot footprint at
    # multiple orientations + differential-drive BFS. Skips the inflation +
    # coarse-grid planner entirely.
    # ═══════════════════════════════════════════════════════════════════════
    if accurate_footprint:
        ...
        L(f"[{label}] Accurate footprint fit done: {time.time()-t0:.2f}s  "
          f"reachable={fp_meta['reachable_cells']}  states={fp_meta['states_expanded']}",
          "success")
        ...
        L(f"[{label}] RII Horizontal = {rii_horizontal:.1f}% (footprint-fit, {fp_meta['orientations']} orientations, {footprint_motion})", "success")

        sx_rep = int((start_x - ox) / res); sy_rep = int((start_y - oy) / res)
        sx_rep = max(0, min(sx_rep, w - 1)); sy_rep = max(0, min(sy_rep, h - 1))
        result = dict(
            coveredArea=accessible_area,
            reachableArea=accessible_area,
            accessibleArea=accessible_area,
        ...
        eff_start_world = (float(start_x), float(start_y))
        result.update(
